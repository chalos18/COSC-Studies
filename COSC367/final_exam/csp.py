...
def scope(constraint):
    ...
    return set(constraint.__code__.co_varnames[: constraint.__code__.co_argcount])
...
def satisfies(assignment, constraint):
    ...
    return constraint(
        **{var: val for var, val in assignment.items() if var in scope(constraint)}
    )

a = satisfies({"a": 1, "b": 2, "c":4}, lambda a, b, c: c > a + b)
print(a)
...
class CSP(collections.namedtuple("CSP", "var_domains, constraints")):
    ...
    def __init__(self, var_domains, constraints):
        ...
        assert all(
            type(name) is str and type(domain) is set
            for name, domain in var_domains.items()
        )
        assert isinstance(constraints, collections.abc.Iterable)
        assert all(
            callable(c) and scope(c) <= set(var_domains.keys()) for c in constraints
        )
