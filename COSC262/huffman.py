...
class HuffmanTree:
    ...
    def find_binary_encoding(self, node, target_char, path=""):
        """Traverse the tree to find the binary encoding for a specific character."""
        # Base case: if the current node is None
        if node is None:
            return None

        # If the node is a leaf and its character matches the target character,
